...
// 사주팔자 가져오기
$input = $_POST['select_id'];
$sql = "
        SELECT * FROM data_storage where id = '{$input}';
        ";
$output_arr = mysqli_fetch_array(mysqli_query($conn, $sql));
    ...
    <body>
        ...
        <div id="explain">
            <p align="center">
                ...
                <!-- 년지, 월지, 일지, 시지 입력-->
                <form action="toyproject3_add.php" method="POST" align="center" name=formm>
                    <br>
                    ...
                    <br>
                    <button type="button" id="btn1" onclick="printt()"> 입력 </button> <!-- 테이블에 지지 출력-->
                    <button type="button" id="btn2" onclick="clearr()"> 초기화 </button> <!-- 테이블 출력부분 초기화-->
                    <button type="button" onclick="logout()"> 로그아웃 </button> <!-- 로그아웃 -->
        ...
        <script type="text/javascript">
            document.getElementById("text_year").value = "<?php echo $output_arr['년주']?>";
            document.getElementById("text_month").value = "<?php echo $output_arr['월주']?>";
            document.getElementById("text_day").value = "<?php echo $output_arr['일주']?>";
            var time = "<?php echo $output_arr['시주'] ?>";
            if (time == null) {
                document.getElementById("text_time").value = "";
            }
            else {
                document.getElementById("text_time").value = "<?php echo $output_arr['시주']?>";
            }
            // 테이블 정렬
